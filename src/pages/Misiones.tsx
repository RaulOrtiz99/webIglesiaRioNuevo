...
// import { Globe, Heart, Award, Users, MapPin } from 'lucide-react';
...
//           style={{ 
//             backgroundImage: "url('https://images.pexels.com/photos/250609/pexels-photo-250609.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2')",
//           }}
//         >
    ...
    return (
      <div className="pt-16 min-h-screen flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-32 w-32 border-b-2 border-celestial-600 mx-auto"></div>
          <p className="mt-4 text-gray-600">Cargando información de misiones...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="pt-16 min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <button 
            ...
            className="bg-celestial-500 hover:bg-celestial-600 text-white px-4 py-2 rounded-md"
          >
            Reintentar
          </button>
        </div>
        ...
        <div className="container mx-auto px-4">
          ...
          {missionaries.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto stagger-anim">
              {missionaries.map((missionary) => (
                ...
                >
                  <div className="h-64 overflow-hidden">
                    <img 
                      src={missionary.imagen_url || 'https://images.pexels.com/photos/3769138/pexels-photo-3769138.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2'} 
                      alt={missionary.nombre} 
                      className="w-full h-full object-cover transition-transform duration-500 hover:scale-110"
                      onError={(e: React.SyntheticEvent<HTMLImageElement>) => {
                        const target = e.target as HTMLImageElement;
                        target.src = 'https://images.pexels.com/photos/3769138/pexels-photo-3769138.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2';
                      }}
                    />
                  </div>
                  <div className="p-6">
                    <div className="flex justify-between items-start mb-3">
                      <h3 className="text-xl font-bold text-gray-800">{missionary.nombre}</h3>
                      <span className="bg-celestial-100 text-celestial-800 text-xs px-2 py-1 rounded-full">
                    ...
                    <div className="flex items-center text-gray-600 mb-4">
                      <MapPin size={16} className="mr-2 text-celestial-500" />
                      <span>{missionary.ubicacion}</span>
                    </div>
                    <p className="text-gray-700 mb-4">{missionary.descripcion}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="text-center py-12">
              <p className="text-gray-600">No hay misioneros registrados en este momento.</p>
            </div>
          )}
        </div>
      </section>
      ...
      <section id="proyectos" className="py-20 bg-white" ref={projectsRef}>
        <div className="container mx-auto px-4">
          ...
          {projects.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto stagger-anim">
              {projects.map((project) => (
                ...
                >
                  <div className="h-48 overflow-hidden">
                    <img 
                      src={project.imagen_url || 'https://images.pexels.com/photos/6647135/pexels-photo-6647135.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2'} 
                      alt={project.titulo} 
                      className="w-full h-full object-cover transition-transform duration-500 hover:scale-110"
                      onError={(e: React.SyntheticEvent<HTMLImageElement>) => {
                        const target = e.target as HTMLImageElement;
                        target.src = 'https://images.pexels.com/photos/6647135/pexels-photo-6647135.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2';
                      }}
                    />
                  </div>
                  <div className="p-6">
                    <div className="flex justify-between items-center mb-3">
                      <h3 className="text-xl font-bold text-gray-800">{project.titulo}</h3>
                    </div>
                    <div className="flex items-center text-gray-600 mb-4">
                      <MapPin size={16} className="mr-2 text-celestial-500" />
                      <span>{project.ubicacion}</span>
                    </div>
                    <p className="text-gray-600 mb-6">{project.descripcion}</p>
                    
                    {project.objetivo && project.recaudado && (
                      <div className="mb-4">
                        <div className="flex justify-between mb-1">
                          <span className="text-sm font-medium text-gray-700">Recaudado: {project.recaudado}</span>
                          <span className="text-sm font-medium text-gray-700">Meta: {project.objetivo}</span>
                        </div>
                        <div className="w-full bg-gray-200 rounded-full h-2.5">
                          <div 
                            className="bg-celestial-500 h-2.5 rounded-full" 
                            style={{ 
                              width: `${Math.min(
                                (parseInt(project.recaudado.replace(/[^\d]/g, '')) / 
                                 parseInt(project.objetivo.replace(/[^\d]/g, ''))) * 100, 
                                100
                              )}%` 
                            }}
                          ></div>
            ...
            <div className="text-center py-12">
              <p className="text-gray-600">No hay proyectos registrados en este momento.</p>
            </div>
          )}
        </div>
      </section>

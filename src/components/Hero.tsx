...
interface HeroSeccion {
  ...
  descripcion: string;
  imagenUrl: string;
  urlUltimaPredica?: string; // Campo opcional
}
      ...
      try {
        ...
        if (json.data && Array.isArray(json.data) && json.data.length > 0) {
          ...
          const mappedData: HeroSeccion = {
            ...
            imagenUrl: heroData.imagenUrl || "",
            // Como urlUltimaPredica no viene en la API, lo omitimos o ponemos undefined
        ...
        <div className="max-w-2xl">
          ...
          >
            ...
            {data.urlUltimaPredica && isValidUrl(data.urlUltimaPredica) ? (
              ...
              // Mensaje informativo cuando no hay URL (opcional - puedes eliminar este bloque si no quieres mostrar nada)
              <div className="text-center">
                <span className="text-yellow-400 block text-sm">
                  📺 Próximamente: Última prédica
                </span>
              </div>
            )}
